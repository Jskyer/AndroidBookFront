...
public class WaterfallUtil extends ScrollView {
    ...
    public WaterfallUtil(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init();
    }

    public WaterfallUtil(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public WaterfallUtil(Context context) {
        super(context);
        init();
    }
        ...
        if (t > oldt) {//向下滚动
            if (t > 2 * viewHeight) {
                for (int i = 0; i < colCount; i++) {
                    ...
                    //回收上方超过两屏bitmap
                    FlowingViewUtil topItem = (FlowingViewUtil) colLayout.getChildAt(currentTopLineIndex[i]);
                    if (topItem.getFootHeight() < t - 2 * viewHeight) {
                        topItem.recycle();
                        currentTopLineIndex[i] ++;
                    }
                    //重载下方进入(+1)两屏以内bitmap
                    FlowingViewUtil bomItem = (FlowingViewUtil) colLayout.getChildAt(Math.min(currentBomLineIndex[i] + 1, bomLineIndex[i]));
            ...
            for (int i = 0; i < colCount; i++) {
                LinearLayout colLayout = colLayoutArray.get(i);
                //回收下方超过两屏bitmap
                FlowingViewUtil bomItem = (FlowingViewUtil) colLayout.getChildAt(currentBomLineIndex[i]);
                if (bomItem.getFootHeight() > t + 3 * viewHeight) {
                    bomItem.recycle();
                    currentBomLineIndex[i] --;
                }
                //重载上方进入(-1)两屏以内bitmap
                FlowingViewUtil topItem = (FlowingViewUtil) colLayout.getChildAt(Math.max(currentTopLineIndex[i] - 1, 0));
                if (topItem.getFootHeight() >= t - 2 * viewHeight) {
                    topItem.reload();
    ...
    private static class DelayHandler extends Handler {
        private WeakReference<WaterfallUtil> WaterfallUtilWR;
        private WaterfallUtil WaterfallUtil;
        public DelayHandler(WaterfallUtil WaterfallUtil) {
            WaterfallUtilWR = new WeakReference<WaterfallUtil>(WaterfallUtil);
            this.WaterfallUtil = WaterfallUtilWR.get();
        }

        @Override
        public void handleMessage(Message msg) {
            //判断当前滑动到的位置，进行不同的处理
            if (WaterfallUtil.getScrollY() + WaterfallUtil.getHeight() >=
                    WaterfallUtil.getMaxColHeight() - 20) {
                //滑动到底部，添加下一页内容
                WaterfallUtil.addNextPageContent(false);
            } else if (WaterfallUtil.getScrollY() == 0) {
                //滑动到了顶部
            } else {
                //滑动在中间位置
            }
    ...
    private static class AddItemHandler extends Handler {
        private WeakReference<WaterfallUtil> WaterfallUtilWR;
        private WaterfallUtil WaterfallUtil;
        public AddItemHandler(WaterfallUtil WaterfallUtil) {
            WaterfallUtilWR = new WeakReference<WaterfallUtil>(WaterfallUtil);
            this.WaterfallUtil = WaterfallUtilWR.get();
        }
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 0x00:
                    FlowingViewUtil FlowingViewUtil = (FlowingViewUtil)msg.obj;
                    WaterfallUtil.addItem(FlowingViewUtil);
                    break;
            }
            super.handleMessage(msg);
        }
    ...
    /**
     * 添加单元到瀑布流中
     * @param FlowingViewUtil
     */
    private void addItem(FlowingViewUtil FlowingViewUtil) {
        int minHeightCol = getMinHeightColIndex();
        colLayoutArray.get(minHeightCol).addView(FlowingViewUtil);
        colHeight[minHeightCol] += FlowingViewUtil.getViewHeight();
        FlowingViewUtil.setFootHeight(colHeight[minHeightCol]);
    ...
    private void addNextPageContent(boolean isFirstPage) {
        ...
        for (int i = pageCount * currentPage;
            ...
            new Thread(new PrepareFlowingViewUtilRunnable(i)).run();
        }
        currentPage ++;
    }

    /**
     * 异步加载要添加的FlowingViewUtil
     * @author carrey
     *
     */
    private class PrepareFlowingViewUtilRunnable implements Runnable {
        private int id;
        public PrepareFlowingViewUtilRunnable (int id) {
            this.id = id;
        }
