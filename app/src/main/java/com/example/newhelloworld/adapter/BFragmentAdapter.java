...
public class BFragmentAdapter extends RecyclerView.Adapter<BFragmentAdapter.ViewHolder> {
    private List<SubscribeInfo> episodeList;
...
//            viewsView = itemView.findViewById(R.id.item_views);
    ...
    public BFragmentAdapter(List<SubscribeInfo> episodeList, Context context) {
    ...
    @NonNull
    @Override
    public BFragmentAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_personal_history, parent, false);
        BFragmentAdapter.ViewHolder holder = new BFragmentAdapter.ViewHolder(inflate);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull BFragmentAdapter.ViewHolder holder, int position) {
        SubscribeInfo episode = episodeList.get(position);
...
//        holder.viewsView.setText("观看 "+ episode.getViews() + " 次");
        ...
        holder.dateView.setText(sdf.format(episode.getCreate_time()));
    ...
    // 暴露接口，更新数据源
    public void updateList(List<SubscribeInfo> newDatas) {
        // 在原有的数据之上增加新数据
